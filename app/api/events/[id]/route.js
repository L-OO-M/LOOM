...
export async function POST(request, { params }) {
  ...
  const [event] = await sql`SELECT * FROM events WHERE id = ${id} AND tenant_id = ${tenant?.id ?? null}::uuid AND status <> 'cancelled' LIMIT 1`;
  if (!event) return fail("NOT_FOUND", "Event not found", 404);

  if (body.action === "cancel") {
    await sql`UPDATE event_registrations SET status = 'cancelled' WHERE event_id = ${id} AND student_id = ${user.id}`;
...
export async function PATCH(request, { params }) {
  ...
  const [row] = await sql`
    ...
    WHERE event_id = ${id} AND student_id = ${user.id} RETURNING *
  `;
