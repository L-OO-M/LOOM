  ...
  return (
    <AppShell area="student" tenant={tenant} user={user}>
      <main className="mx-auto max-w-3xl px-4 sm:px-6">
        <Link href="/student/roadmap" className="meta hover:underline" style={{ color: "var(--accent)" }}>← The path</Link>
        <p className="meta mt-6">
          {node.domain}{node.difficulty_level ? ` · ${node.difficulty_level}` : ""} · {completed ? "Completed" : "In progress"}
        </p>
          ...
          <ol className="mt-5">
            {resources.map((r, i) => (
              <li key={r.id} className="border-b py-4 first:border-t" style={{ borderColor: "var(--line)" }}>
                <Link href={`/student/resources/${r.id}`} className="row-link flex items-baseline gap-4 px-2 py-1">
        ...
        <nav className="mt-10 flex items-center justify-between gap-3 border-t pt-6" style={{ borderColor: "var(--line)" }} aria-label="Path">
          {prev ? (
            <Link href={`/student/roadmap/${prev.id}`} className="text-sm font-semibold hover:underline" style={{ color: "var(--text)" }}>
              ← {prev.title}
            </Link>
          ) : <span />}
          {next ? (
            <Link href={`/student/roadmap/${next.id}`} className="text-sm font-semibold hover:underline" style={{ color: "var(--accent)" }}>
              {next.title} →
            </Link>
          ) : <span />}
        </nav>
