export default function DepartmentsLoading() {
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-44 animate-pulse rounded-2xl border" style={{ borderColor: "var(--line)", background: "var(--bg-elevated)" }} />)}</div>
    </div>
  );
}
