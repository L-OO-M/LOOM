...
export default async function AdminPage() {
  ...
  // Every aggregate below depends only on tid → one concurrent wave instead
  // of ~16 serial Singapore round-trips. Query text is unchanged.
  const [
    studentRows, startedRows, beginnersActiveRows, completionRows, mergeRows,
    projectRows, submissionRows, mentorRows, eventsHeldRows, partnershipRows,
    flaggedRows, claimRows, draftRows, applicationRows, upcoming, auditEntries
  ] = await Promise.all([
    sql`SELECT COUNT(*)::int AS c FROM profiles WHERE role = 'student' AND (tenant_id = ${tid} OR ${tid}::uuid IS NULL)`,
